...
const TechSkills = () => {
  const icons = [
    ...
    {name: 'Next.js', color: 'text-neutral-50'},
    ...
    {name: 'Gitea', color: 'text-green-500'},
  ...
  const TechIcon = ({ icon }) => (
    <div className="px-2">
      <div className="flex w-full space-x-2 rounded-full border border-neutral-200 dark:border-neutral-600 px-4 py-2 shadow-sm bg-neutral-50 dark:bg-[#1D293D]">
        ...
        <div className="whitespace-nowrap">{icon.name}</div>
      </div>
    </div>
  );
    ...
    <>
      ...
      <div className="relative overflow-hidden rounded-2xl border-[1.5px] border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 !p-0 md:col-span-1 flex flex-col from-emerald-400 to-green-600">
        ...
        <div className="flex flex-col overflow-x-hidden">
          <div className="tech-row row1">
            {icons.slice(0,4).map((icon, index) => <TechIcon key={index} icon={icon} />)}
          </div>
          <div className="tech-row row2">
            {icons.slice(4,8).map((icon, index) => <TechIcon key={index} icon={icon} />)}
          </div>
          <div className="tech-row row3">
            {icons.slice(8,13).map((icon, index) => <TechIcon key={index} icon={icon} />)}
          </div>
        </div>
      </div>
    </>
