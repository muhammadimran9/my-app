...
const GitHubContributions = ({ username }) => {
        ...
        <div className="animate-pulse space-y-4">
          ...
          <div className="grid grid-cols-53 gap-1">
            {Array.from({ length: 371 }).map((_, i) => (
              <div key={i} className="h-3 bg-neutral-300 dark:bg-neutral-700 rounded"></div>
            ))}
          </div>
        </div>
      </div>
    ...
    return (
      <div className="rounded-2xl border-[1.5px] border-neutral-300 bg-neutral-100 dark:border-neutral-800 dark:bg-neutral-900 p-6">
        <p className="text-neutral-600 dark:text-neutral-400">
          Unable to load GitHub contributions. Please check your API configuration.
        </p>
      </div>
    );
  }
  ...
  // Get contribution levels for color mapping
      ...
      <div className="overflow-x-auto">
        <div className="inline-block min-w-full">
          <div className="flex gap-1 relative">
            {/* Day labels */}
            <div className="flex flex-col gap-1 mr-2 pt-3">
              {dayLabels.map((day, idx) => (
                <div
                  key={idx}
                  className="text-xs text-neutral-600 dark:text-neutral-400 h-3"
            ...
            <div className="flex-1 relative">
              ...
              <div className="absolute -top-5 left-0 right-0 flex">
                {monthPositions.map(({ month, weekIndex }, idx) => (
                  <div
                    key={idx}
                  ...
                  <div key={weekIndex} className="flex flex-col gap-1">
                    {week.contributionDays.map((day, dayIndex) => {
                      ...
                      return (
                        <div
                          key={dayIndex}
                          className={`w-3 h-3 rounded ${getColor(level)} transition-colors hover:ring-2 hover:ring-neutral-400 dark:hover:ring-neutral-600 cursor-pointer`}
                          title={`${day.contributionCount} contributions on ${date.toLocaleDateString()}`}
                        />
                      );
                    })}
                  </div>
