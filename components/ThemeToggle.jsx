...
export default function ThemeToggle({ className = '' }) {
  ...
  return (
    <button
      onClick={toggleTheme}
      className={`relative flex items-center gap-2 rounded-full border-[1.5px] p-1 transition-all duration-300 ${className}`}
      style={{ 
    ...
    >
      <div 
        className="absolute bottom-1 top-1 w-8 rounded-full transition-transform duration-300" 
        style={{ 
      ...
      <div 
        ...
        style={{ color: !isDark ? 'rgb(255, 255, 255)' : 'var(--text-muted)' }}
      >
